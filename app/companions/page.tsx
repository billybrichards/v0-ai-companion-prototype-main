...
import { Lock, Shield, MessageSquare, Mic, Sparkles, Check, Crown, User, Image as ImageIcon, Video, Headphones, Star, ChevronLeft, ChevronRight } from "lucide-react"
...
// Hook images for carousel and floating effects
const hookImages = Array.from({ length: 25 }, (_, i) =>
  `/images/hooks/hook_img_${String(i + 1).padStart(2, '0')}.png`
)

// Testimonial images
const testimonialImages = Array.from({ length: 25 }, (_, i) =>
  `/images/testimonials/test_img_${String(i + 1).padStart(2, '0')}.png`
)

// Visual Showcase Carousel Component
function VisualShowcaseCarousel() {
  ...
  return (
    <div className="relative group">
      ...
      <button
        onClick={() => scroll('left')}
        className={`absolute left-2 top-1/2 -translate-y-1/2 z-10 h-10 w-10 rounded-full bg-background/80 backdrop-blur-sm border border-border/50 flex items-center justify-center transition-all duration-300 ${
          canScrollLeft ? 'opacity-100 hover:bg-primary/20 hover:border-primary/50' : 'opacity-0 pointer-events-none'
        }`}
      ...
      >
        {hookImages.slice(0, 15).map((src, index) => (
          <div
            key={index}
            className="relative flex-shrink-0 w-[280px] h-[380px] rounded-2xl overflow-hidden snap-center group/card"
          >
            <Image
              src={src}
              alt={`Visual showcase ${index + 1}`}
              fill
            ...
            <div className="absolute bottom-4 left-4 right-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-foreground/80 font-medium">Explore #{index + 1}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <button
        ...
        className={`absolute right-2 top-1/2 -translate-y-1/2 z-10 h-10 w-10 rounded-full bg-background/80 backdrop-blur-sm border border-border/50 flex items-center justify-center transition-all duration-300 ${
          canScrollRight ? 'opacity-100 hover:bg-primary/20 hover:border-primary/50' : 'opacity-0 pointer-events-none'
        }`}
...
// Floating Images Component for parallax effects
function FloatingImages() {
  ...
  const floatingPositions = [
    { top: '10%', left: '5%', size: 80, delay: 0, parallaxFactor: 0.05, imgIndex: 16 },
    { top: '25%', right: '3%', size: 100, delay: 1, parallaxFactor: 0.08, imgIndex: 17 },
    { top: '45%', left: '2%', size: 70, delay: 2, parallaxFactor: 0.03, imgIndex: 18 },
    { top: '60%', right: '5%', size: 90, delay: 0.5, parallaxFactor: 0.06, imgIndex: 19 },
    { top: '75%', left: '4%', size: 85, delay: 1.5, parallaxFactor: 0.04, imgIndex: 20 },
    { top: '85%', right: '2%', size: 75, delay: 2.5, parallaxFactor: 0.07, imgIndex: 21 },
  ]
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      {floatingPositions.map((pos, index) => (
        <div
          key={index}
          className="absolute opacity-20 rounded-full overflow-hidden animate-float"
          style={{
            top: pos.top,
            left: pos.left,
            right: pos.right,
            width: pos.size,
            height: pos.size,
            transform: `translateY(${scrollY * pos.parallaxFactor}px)`,
            animationDelay: `${pos.delay}s`,
          }}
        >
          <Image
            src={hookImages[pos.imgIndex] || hookImages[0]}
            alt=""
            fill
...
function TestimonialCard({
  ...
  return (
    <div className="p-4 rounded-xl bg-background/50 border border-border/30 hover:border-primary/30 transition-all duration-300">
      <div className="flex items-start gap-3 mb-3">
        <div className="relative h-10 w-10 rounded-full overflow-hidden flex-shrink-0 ring-2 ring-primary/30">
          <Image
            src={testimonialImages[imageIndex] || testimonialImages[0]}
            alt={name}
            fill
            className="object-cover"
            sizes="40px"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 mb-1">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`h-3 w-3 ${i < rating ? 'text-yellow-500 fill-current' : 'text-muted fill-current'}`}
              />
            ))}
          </div>
          <span className="text-xs font-medium truncate block">{name} in {location}</span>
        </div>
      </div>
...
function TestimonialImageGrid() {
  return (
    <div className="grid grid-cols-5 gap-2 mb-6">
      {testimonialImages.slice(0, 5).map((src, index) => (
        <div
          key={index}
          className="relative aspect-square rounded-lg overflow-hidden ring-2 ring-transparent hover:ring-primary/50 transition-all duration-300"
        >
          <Image
            src={src}
...
export default function CompanionsLandingPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-foreground relative">
      ...
      <main className="pt-16 sm:pt-20 relative z-10">
        ...
        {/* Visual Showcase Section with Carousel */}
        <section className="px-0 py-10 sm:py-14 md:py-18 bg-gradient-to-b from-transparent via-card/20 to-transparent">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 mb-6 sm:mb-8">
            <div className="text-center">
              <h2 className="font-heading text-xl sm:text-2xl md:text-3xl font-light mb-2 sm:mb-3">
                <span className="text-primary">Discover</span> Your Connection
              </h2>
              <p className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto">
                Explore a world of personalized AI companions designed to understand you
              </p>
            </div>
          </div>
        ...
        <section className="px-4 sm:px-6 py-12 sm:py-16 md:py-20 bg-card/10">
          <div className="mx-auto max-w-6xl">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              {/* Testimonial Visual - Enhanced with Images */}
              <div className="flex flex-col items-center md:items-start p-6 rounded-3xl bg-card/30 border border-border/50">
                ...
                <div className="space-y-4 w-full">
                  <TestimonialCard
                    ...
                    location="Shiplake"
                    rating={4}
                    text="It's been helpful to have someone to chat with, especially when I'm feeling a bit bored or lonely. The conversations feel natural..."
                    imageIndex={5}
                  />
                  <TestimonialCard
                    name="Darcy"
                    location="New York"
                    rating={4}
                    text="I recently started using an AI companion to help manage my bipolar disorder... discovered it has some surprising benefits."
                    imageIndex={6}
                  />
                  <TestimonialCard
                    name="Marcus"
                    location="London"
                    rating={5}
                    text="Finally an AI that doesn't judge. I can be myself here, explore thoughts I'd never share elsewhere. Truly liberating."
                    imageIndex={7}
                  />
                </div>
              </div>
        ...
        {/* Secondary Auto-Scrolling Image Carousel */}
        <section className="py-8 sm:py-12 overflow-hidden bg-gradient-to-r from-primary/5 via-transparent to-primary/5">
          <div className="flex animate-scroll-left">
            {[...hookImages.slice(5, 15), ...hookImages.slice(5, 15)].map((src, index) => (
              <div
                key={index}
                className="flex-shrink-0 w-[150px] h-[150px] mx-2 rounded-xl overflow-hidden opacity-60 hover:opacity-100 transition-opacity duration-300"
              >
                <Image
                  src={src}
                  alt=""
        ...
        <section className="px-4 sm:px-6 py-8 sm:py-10 md:py-14">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
              {[
                { icon: ImageIcon, label: "Image Generation", description: "Visual experiences" },
        ...
        {/* Hook Image Feature Grid */}
        <section className="px-4 sm:px-6 py-10 sm:py-14 md:py-18">
          <div className="mx-auto max-w-6xl">
            <div className="text-center mb-8 sm:mb-12">
              <h2 className="font-heading text-xl sm:text-2xl md:text-3xl font-light mb-3">
                Every Conversation, <span className="text-primary">Uniquely Yours</span>
              </h2>
              <p className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto">
                Your companion adapts to you - your mood, your desires, your boundaries
              </p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
              {hookImages.slice(0, 8).map((src, index) => (
                <div
                  key={index}
      ...
      <footer className="border-t border-border/50 px-4 sm:px-6 py-6 sm:py-8 safe-bottom relative z-10">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col items-center justify-between gap-4 sm:gap-6 md:flex-row">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm ml-1" title="Made in the UK">GB</span>
            </div>
